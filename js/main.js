...
const icons = [];
icons.push(document.querySelector("#umbrella"));
icons.push(document.querySelector("#battery"));
icons.push(document.querySelector("#bag"));
    ...
    fetch(`${baseUrl}/location`)
        ...
        .then(locations => {
            locations.forEach(location => {
                ...
                const availableIcons = icons.filter(e => location.items.includes(e.id));
                tuneMarker(marker, map, geocoder, availableIcons);
            })
        })
        .catch(err => console.log(err));
}

function tuneMarker(marker, map, geocoder, icons) {
    const infoWindow = new google.maps.InfoWindow();

    geocoder.geocode({'location': marker.position}, (results, status) => {
        if (status === 'OK') {
            infoWindow.setContent(`${results[0].formatted_address}<br>${composeActiveIcons(icons)}`);
        }
    });
...
function getActiveIcons(icons) {
    return icons.filter(e => e.style.color === 'black')
}

function composeActiveIcons(icons) {
    return getActiveIcons(icons).map(e => e.outerHTML).join(' ');
}